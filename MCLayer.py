...
def compute_temperature(pre_sigmoid_temperature, lower: Optional[float],
    ...
    temperature = F.sigmoid(pre_sigmoid_temperature)
...
class MCSoftmaxOutputLayerBase(nn.Module):
    ...
    def __init__(self,
        ...
        # after applying the sigmoid transformation.
            self._pre_sigmoid_temperature = nn.Parameter(torch.randn(1))
        else:
    ...
    def _compute_noise_samples(self, scale, num_samples, seed):
        ...
        return torch.transpose(noise_samples, [1,0,2]) * scale.unsqueeze(1)
    ...
    def _compute_predictive_mean(self, locs, scale, total_mc_samples, seed):
        ...
        return torch.mean(samples, dim=1), samples
    ...
    def _compute_predictive_variance(self, mean, locs, scale, seed, num_samples):
        ...
        total_variance = torch.mean((mc_samples-mean)**2, dim=1)
    ...
    def __call__(self, inputs, training=True, seed=None):
        ...
        probs_mean = torch.clip(probs_mean, min=self._eps, max=1.0)
        log_probs = torch.log(probs_mean)

        if self._num_classes == 2:
            # inverse sigmoid
            probs_mean = torch.clip(probs_mean, min=self._eps, max=1.0 - self._eps)
            logits = log_probs - torch.log(1.0 - probs_mean)
        else:
            logits = log_probs

        if self._return_unaveraged_logits:
            samples = torch.clip(samples, min=self._eps, max=1.0)
            samples_log_probs = torch.log(samples)

            if self._num_classes == 2:
                # inverse sigmoid
                samples = torch.clip(samples, min=self._eps, max=1.0 - self._eps)
                samples_logits = samples_log_probs - torch.log(1.0 - samples)
            else:
...
class MCSoftmaxDense(MCSoftmaxOutputLayerBase):
    ...
    def __init__(self,
        ...
        self._loc_layer = nn.Linear(1 if num_classes == 2 else num_classes)
        self._scale_layer = nn.Linaer(1 if num_classes == 2 else num_classes)
    ...
    def _compute_scale_param(self, inputs):
        ...
        Tensor of shape [batch_size, num_classes].
        """
        return self.activation(self._scale_layer(inputs)) + MIN_SCALE_MONTE_CARLO

    def get_config(self):
        config = {
            'loc_layer': self._loc_layer.state_dict(),
            'scale_layer': self._scale_layer.state_dict(),
        }
        new_config = super().get_config()
        new_config.update(config)
        return new_config
...
class MCSoftmaxDenseFA(MCSoftmaxOutputLayerBase):
    ...
    def __init__(self,
        ...
        super(MCSoftmaxDenseFA, self).__init__(
            num_classes, logit_noise=Normal,
        ...
        if parameter_efficient:
            self._scale_layer_homoscedastic = nn.Linear(in_dim, num_classes)
            self._scale_layer_heteroscedastic = nn.Linear(in_dim, num_classes)
        else:
            self._scale_layer = nn.Linear(in_dim, num_classes*num_factors)

        self._loc_layer = nn.Linear(in_dim, num_classes)
        self._diag_layer = nn.Linear(in_dim, num_classes)
    ...
    def _compute_diagonal_noise_samples(self, diag_scale, num_samples, seed):
        ...
        return diag_noise_samples * torch.unsqueeze(diag_scale, 1)
    ...
    def _compute_noise_samples(self, scale, num_samples, seed):
        ...
        if self._parameter_efficient:
            ...
            res *= self._scale_layer_heteroscedastic(factor_loadings).unsqueeze(1)
        else:
            # reshape scale vector into factor loadings matrix
            factor_loadings = factor_loadings.reshape(-1, self._num_classes, self._num_factors)
    ...
    def get_config(self):
        ...
        if self._parameter_efficient:
            config['scale_layer_homoscedastic'] = self._scale_layer_homoscedastic.state_dict()
        else:
            config['scale_layer'] = self._scale_layer.state_dict()
...
class MultiHeadMCSoftmaxDenseFA(MCSoftmaxOutputLayerBase):
    ...
    def __init__(self, num_classes, num_factors, ensemble_size, temperature=1.0,
        ...
        if parameter_efficient:
            self._scale_layer_homoscedastic = nn.Linear(num_classes * ensemble_size)
            self._scale_layer_heteroscedastic = nn.Linear(num_classes * ensemble_size)
        else:
            self._scale_layer = nn.Linear(num_classes * ensemble_size * num_factors)
            self._loc_layer = nn.Linear(num_classes * ensemble_size)
            self._diag_layer = nn.Linear(num_classes * ensemble_size)
    ...
    def _compute_mc_samples(self, locs, scale, num_samples, seed):
        ...
        noise_samples = self._compute_noise_samples(scale, num_samples, seed)
        latents = locs + noise_samples
        latents = latents.reshape(num_samples, self._ensemble_size, self._num_classes)
        if self._num_classes == 2:
            return F.sigmoid(latents / self._temperature)
        else:
            return torch.softmax(latents / self._temperature)
    ...
    def _compute_diagonal_noise_samples(self, diag_scale, num_samples, seed):
        ...
        dist = Normal(
            loc=torch.zeros([num_noise_samples,
                        self._num_classes * self._ensemble_size],
                        dtype=diag_scale.dtype),
            scale=torch.ones([num_noise_samples,
                        self._num_classes * self._ensemble_size],
                        dtype=diag_scale.dtype))
        ...
        # [total_mc_samples, batch_size, d], here we reshape to
        # [batch_size, total_mc_samples, d]
        diag_noise_samples = diag_noise_samples.transpose(1, 0, 2)
    ...
    def _compute_standard_normal_samples(self, factor_loadings, num_samples,
        ...
        # [total_mc_samples, batch_size, d], here we reshape to
        # [batch_size, total_mc_samples, d]
        tandard_normal_samples = standard_normal_samples.transpose(1, 0, 2)
    ...
    def _compute_noise_samples(self, scale, num_samples, seed):
        ...
        if self._parameter_efficient:
            res = self._scale_layer_homoscedastic(standard_normal_samples)
            res *= self._scale_layer_heteroscedastic.unsqueeze(1)
        else:
            # reshape scale vector into factor loadings matrix
            factor_loadings = factor_loadings.reshape(-1, self._num_classes * self._ensemble_size, self._num_factors)
    ...
    def get_config(self):
        ...
        if self._parameter_efficient:
            config['scale_layer_homoscedastic'] = self._scale_layer_homoscedastic.state_dict()
            config['scale_layer_heteroscedastic'] = self._scale_layer_heteroscedastic
        else:
            config['scale_layer'] = self._scale_layer
...
class MCSoftmaxDenseFACustomLayers(MCSoftmaxOutputLayerBase):
    ...
    def _compute_standard_normal_samples(self, factor_loadings, num_samples,
        ...
        # [total_mc_samples, batch_size, d], here we reshape to
        # [batch_size, total_mc_samples, d]
        standard_normal_samples = standard_normal_samples.transpose(1, 0, 2)
    ...
    def get_config(self):
        ...
        if self._parameter_efficient:
            config['scale_layer_homoscedastic'] = self._scale_layer_homoscedastic.state_dict()
            config['scale_layer_heteroscedastic'] = self._scale_layer_heteroscedastic.state_dict()
        else:
            config['scale_layer'] = self._scale_layer.state_dict()
...
class MCSigmoidDenseFA(MCSoftmaxOutputLayerBase):
    ...
    def __init__(self,
        ...
        self._loc_layer = nn.Linear(num_outputs)

        if num_factors > 0:
            if parameter_efficient:
                self._scale_layer_homoscedastic = nn.Linear(num_outputs, num_outputs)
                self._scale_layer_heteroscedastic = nn.Linear(num_outputs, num_outputs)
            else:
                self._scale_layer = nn.Linear(num_outputs * num_factors)

        self._diag_layer = nn.Linear(num_outputs)
    ...
    def _compute_diagonal_noise_samples(self, diag_scale, num_samples, seed):
        ...
        dist = Normal(
            loc=torch.zeros([num_noise_samples, self._num_outputs], diag_scale.dtype),
            scale=torch.ones([num_noise_samples, self._num_outputs], diag_scale.dtype))
    ...
    def _compute_standard_normal_samples(self, factor_loadings, num_samples,
        ...
        # [total_mc_samples, batch_size, d], here we reshape to
        # [batch_size, total_mc_samples, d]
        standard_normal_samples = torch.transpose(standard_normal_samples, [1, 0, 2])
    ...
    def _compute_noise_samples(self, scale, num_samples, seed):
        ...
        if self._parameter_efficient:
            ...
            noise_samples *= self._scale_layer_heteroscedastic(factor_loadings).unsqueeze(1)
            return noise_samples + diag_noise_samples
        else:
            # reshape scale vector into factor loadings matrix
            factor_loadings = factor_loadings.reshape(
                                   [-1, self._num_outputs, self._num_factors])

            # transform standard normal into ~ full rank covariance Gaussian samples
            noise_samples = torch.einsum('ijk,iak->iaj', factor_loadings,
    ...
    def get_config(self):
        config = {
            ...
            'parameter_efficient': self._parameter_efficient,
            'loc_layer': self._loc_layer.state_dict(),
            'diag_layer': self._diag_layer.state_dict(),
        }
...
class ExactSigmoidDense(nn.Module):
    """Exact diagonal covariance method for binary/multilabel classification."""

    def __init__(self, num_outputs, logit_noise=Normal,
        ...
        """
        super(ExactSigmoidDense, self).__init__()

        if logit_noise not in (Normal):
            raise ValueError('logit_noise must be Normal or Logistic')

        self._loc_layer = nn.Linear(num_outputs, num_outputs)

        self._diag_layer = nn.Linear(num_outputs, num_outputs)
        self.activation = nn.SoftPlus()
    ...
    def __call__(self, inputs, training=True):
        ...
        dist = self._logit_noise(loc=torch.constant(0.0, dtype=torch.float32),
                               scale=torch.constant(1.0, dtype=torch.float32))
        ...
        if self._logit_noise == tfp.distributions.Normal:
            ...
            probs = F.sigmoid(loc / scale)
            log_probs = F.log_sigmoid(loc / scale)
            logits = loc / scale
...
class EnsembleHeteroscedasticOutputs(nn.Module):
    ...
    def __call__(self, inputs, training=True):
        ...
        """
        with torch.name_scope(self._name):
            if self._averaging == 'ensemble_cross_ent':
                log_weights = torch.log(self._ensemble_weighting)
                ...
                log_weight = log_weight.type(member_log_prob.dtype)
                weighted_log_probs.append((log_weight+member_log_prob).type(torch.float32))

                weighted_log_probs = torch.stack(weighted_log_probs, axis=-1)

                log_probs = torch.math.reduce_logsumexp(weighted_log_probs, axis=-1)

                probs = torch.exp(log_probs)
                probs = torch.clip(probs, self._eps, 1.0 - self._eps)
                if self._num_classes == 2:
                    # inverse sigmoid to transform probs to logits for sigmoid cross-ent
                    logits = log_probs - torch.log(1.0 - probs)
                else:
                    logits = log_probs
            else:
                logits = 0.0
                ...
                for i, layer in enumerate(self._layers):
                    ...
                    weight = weight.type(member_logits.dtype)
                    logits += (weight * member_logits).type(torch.float32)
                    probs += (weight * member_results[2]).type(torch.float32)
...
class MCSoftmaxDenseFASegmentation(MCSoftmaxDenseFA):
    ...
    def _genrate_4d_standard_normal_samples(self,
                                          dimensions: Collection[torch.Tensor],
        ...
        dist = Normal(
            loc=torch.zeros(dimensions, dtype=dtype),
            scale=torch.ones(dimensions, dtype=dtype))
        ...
        standard_normal_samples = standard_normal_samples.transpose([1, 0, 2, 3, 4])
        return standard_normal_samples

    def _compute_diagonal_noise_samples(self, diag_scale: torch.tensor,
                                      num_samples: int, seed: int) -> torch.tensor:
        ...
        height, width = diag_scale.shape[1:3]
    ...
    def _compute_standard_normal_samples(self, factor_loadings: torch.tensor,
                                       num_samples: int,
        ...
        height, width = factor_loadings.shape[1:3]
        ...
        if self._share_samples_across_batch:
            return torch.tile(
                standard_normal_samples, [torch.shape(factor_loadings)[0], 1, 1, 1, 1])
        else:
            return standard_normal_samples

    def _compute_noise_samples(self, scale: Collection[torch.tensor],
                             num_samples: int, seed: int) -> torch.tensor:
        ...
        if self._parameter_efficient:
            ...
            # reshape scale vector into factor loadings matrix
            batch_size, height, width = factor_loadings.shape[:3]
            factor_loadings = factor_loadings.reshape([batch_size, height, width, self._num_classes, self._num_factors])

        # transform standard normal into ~ full rank covariance Gaussian samples
        res = torch.einsum('ihwjk,iahwk->iahwj', factor_loadings,
                      standard_normal_samples)
        return res + diag_noise_samples
